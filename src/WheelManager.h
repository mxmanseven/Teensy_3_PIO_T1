...
class WheelManager
{
  ...
  static uint16_t MIN_MS_BETWEEN_TICKS;
  ...
  static float GetSpeed(uint8_t durationSeconds);
  ...
  static void AddTickRaw();

  static float miliageAdjustment;

    // used for resets
  static void AddDistance(float milesAdjustment);

  static void ChangeDistanceAndCalibrate(float milesAdjustment);
